...
function RegisteredLearners() {
  const [data, setData] = useState([]);
  useEffect(() => {
    async function fetchMyAPI() {
      let result = await fetch("http://localhost:8000/api/learnerslist");
  ...
  async function deleteOperation(id) {
    let result = await fetch("http://localhost:8000/api/deletelearner/" + id, {
      method: "DELETE",
    });
    result = await result.json();
    getData();
...
export default RegisteredLearners;
